...
class PromptDialog(QtWidgets.QDialog, uic.loadUiType(DIALOG_UI_PATH)[0]):
    ...
    def __init__(self, parent=None, appicon=None):
        ...
        # Add Reset to Defaults button next to Allow button
        self.resetButton = QtWidgets.QPushButton()
        self.resetButton.setText("↺")
        self.resetButton.setToolTip(QC.translate("popups", "Reset to Defaults"))
        self.resetButton.setFixedSize(30, 24)
        self.resetButton.clicked.connect(self._cb_reset_to_defaults)
        
        # Try to add to the button bar layout
        try:
            button_layout = None
            # The button bar is in the horizontalLayout
            if hasattr(self, 'horizontalLayout'):
                button_layout = self.horizontalLayout
            if button_layout is None:
                # Get layout from allowButton's parent
                button_layout = self.allowButton.parent().layout()
            if button_layout:
                button_layout.addWidget(self.resetButton)
        except Exception as e:
            print(f"Failed to add reset button: {e}")
    ...
    def _apply_saved_choices(self):
        ...
        # Apply saved "Advanced" checkbox state (default to False if not set)
        last_advanced = self._cfg.getBool(self._cfg.DEFAULT_POPUP_LAST_ADVANCED, False)
        self.checkAdvanced.setChecked(last_advanced)
        self._check_advanced_toggled(last_advanced)
        
        # Apply saved checkbox states (default to False if not set)
        last_dstip = self._cfg.getBool(self._cfg.DEFAULT_POPUP_LAST_DSTIP, False)
        self.checkDstIP.setChecked(last_dstip)
        
        last_dstport = self._cfg.getBool(self._cfg.DEFAULT_POPUP_LAST_DSTPORT, False)
        self.checkDstPort.setChecked(last_dstport)
        
        last_userid = self._cfg.getBool(self._cfg.DEFAULT_POPUP_LAST_USERID, False)
    ...
    def _send_rule(self):
        try:
            ...
            # Remember user's choices for next time
            self._cfg.setSettings(self._cfg.DEFAULT_POPUP_LAST_ACTION, self._default_action)
            self._cfg.setSettings(self._cfg.DEFAULT_POPUP_LAST_DURATION, self.durationCombo.currentIndex())
            self._cfg.setSettings(self._cfg.DEFAULT_POPUP_LAST_TARGET, self.whatCombo.currentIndex())
            # Remember advanced checkbox choices
            self._cfg.setSettings(self._cfg.DEFAULT_POPUP_LAST_ADVANCED, self.checkAdvanced.isChecked())
